#!/usr/bin/env python3
# criar_partidas_teste.py - SCRIPT PARA CRIAR PARTIDAS DE TESTE LOCALMENTE
...
def criar_partidas_teste():
    """Cria partidas de teste para todas as combinacoes de campo, plano e tempo"""
    ...
    with app.app_context():
        try:
            ...
            if not tete:
                ...
                db.session.add(tete)
                db.session.flush()
                print(f"  OK Tete criado (ID: {tete.id})")
            else:
                print(f"  OK Tete ja existe (ID: {tete.id})")
            
            db.session.commit()
            print("  OK Operadores salvos no banco")
            ...
            for idx, config in enumerate(partidas_criar, 1):
                ...
                # Print a cada 10 partidas para acompanhamento
                if idx % 10 == 0:
